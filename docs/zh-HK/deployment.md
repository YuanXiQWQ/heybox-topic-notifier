...
Deno Deploy App 已綁定 Deno KV 資料庫。代碼通過 `Deno.openKv()`
...
發布時間列表；`web` 僅保留為診斷回退。新帳戶預設關閉輪詢，
只有帳戶在設定頁開啟「啟用輪詢」後才會實際擷取。

## 通知中轉

如果 Deno Deploy 不能直接訪問 PushPlus、WxPusher 或 Server醬，可以先部署免費的 Cloudflare Worker
